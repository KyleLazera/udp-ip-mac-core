 ...
 *    Type of Service - This field is mainly used for routers and will be set to 0 for this project
...
#(
   ...
   input wire [47:0] ip_tx_src_mac_addr,                                // Source MAC address
   input wire [47:0] ip_tx_dst_mac_addr,                                // Destination MAC address
...
reg [47:0] ip_hdr_src_mac_addr;                               
reg [47:0] ip_hdr_dst_mac_addr; 

// IP Header Constants
assign ip_hdr_version = IPv4_VERSION;
assign ip_hdr_length = IPv4_HDR_LENGTH;
assign ip_hdr_type = IPv4_TYPE;
      ...
      case(state_reg)
         IDLE : begin
            ...
            if(ip_tx_hdr_valid & ip_tx_hdr_rdy_reg) begin
               ...
               ip_hdr_src_mac_addr <= ip_tx_src_ip_addr;
               ip_hdr_dst_mac_addr <= ip_tx_dst_mac_addr;
               //initially set checksum to zero, until it is calculated
         ...
         HEADER : begin
            ...
            case(ip_hdr_cnt)
               5'd0: begin
                  ...
                  ip_hdr_checksum_field <= {ip_hdr_version, ip_hdr_length, ip_hdr_type};
               end
               5'd1: begin
               ...
               5'd2: begin
                  ...
                  ip_hdr_checksum_field <= ip_hdr_id;              
               end
               5'd3: begin
         ...
         PAYLOAD : begin
            ...
            if(s_tx_axis_trdy_reg & s_tx_axis_tvalid & m_tx_axis_trdy) begin
               ...
               //If we jsut sampled the final byte of data, return to the IDLE state on the next
               // clock edge
               if(s_tx_axis_tlast & m_tx_axis_tvalid_reg)
                  state_reg <= IDLE;
