...
# MAC to the PHY.
# For the output delay - the maximum setup time 
#-------------------------------------------------------------------------

set tx_oddr_cells [get_cells -hierarchical -filter {NAME =~ "*data_oddr*genblk1[*].ODDR_inst"}]

# RGMII PHY has typical setup time of 2ns
...
#RGMII PHY has minimum hold time of 1.2 ns
set_output_delay -clock rgmii_phy_txc -min 0.0 [get_ports {rgmii_phy_txd[*]}]
set_output_delay -clock rgmii_phy_txc -min 0.0 [get_ports {rgmii_phy_txd[*]}] -clock_fall -add_delay

set_output_delay -clock rgmii_phy_txc -min 0.0 [get_ports rgmii_phy_txctl]
set_output_delay -clock rgmii_phy_txc -min 0.0 [get_ports rgmii_phy_txctl] -clock_fall -add_delay